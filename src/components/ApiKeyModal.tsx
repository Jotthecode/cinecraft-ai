...
export const ApiKeyModal: React.FC<ApiKeyModalProps> = ({
  ...
  const [preferredModel, setPreferredModel] = useState<ApiKeys['preferredModel']>(
    apiKeys.preferredModel || 'gemini-2.5-flash-image'
  );
        ...
        <div className="flex items-center justify-between border-b border-slate-800 bg-slate-950/60 px-6 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Configure LLM parsing & image engine choices</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSave} className="space-y-4 p-6 text-sm">
          ...
          <div className="space-y-1.5 rounded-xl border border-indigo-500/30 bg-indigo-950/20 p-3.5 ring-1 ring-indigo-500/20">
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-1.5 text-xs font-bold text-indigo-300">
                ...
                <span>Google Gemini API Key (Gemini 2.5 Flash Image)</span>
              </label>
              <a
            ...
            <p className="text-[11px] text-slate-400 leading-tight">
              Recommended: Powers Gemini 2.5 Flash Image for image-conditioned generation & editing.
            </p>
          </div>
          ...
          <div className="space-y-2 pt-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { id: 'gemini-2.5-flash-image', name: 'Gemini 2.5 Flash Image', badge: 'Primary Multimodal' },
          ...
          <div className="flex items-start gap-2 rounded-xl bg-slate-950/60 p-3 ring-1 ring-slate-800">
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              <strong className="text-slate-200">Image-Conditioned Pipeline: </strong>
              Image generation uses Gemini 2.5 Flash Image & FLUX Kontext image conditioning to maintain character visual identity across panels.
            </p>
          </div>
